...
// how big is address space?
    ...
    ASSERT(numPages <= espacioDisponible);		// check we're not trying
...
// first, set up the translation 
	...
	//copy all segments to memory
	
	//por donde voy en memoria y por donde voy en el archivo.
	int n = noffH.code.size + noffH.initData.size;
    unsigned cantNumPages = divRoundUp(n, PageSize);

	int cantLeida = 0;
	for(unsigned j=0; j<cantNumPages; ++j){
		if(cantLeida+PageSize <= noffH.code.size){
			//lee codigo pagina completa
			executable->ReadAt(&(machine->mainMemory[pageTable[j].physicalPage*PageSize]), PageSize, noffH.code.inFileAddr+cantLeida);
			cantLeida += PageSize;
			pageTable[j].readOnly = true;		
		}else{
			//paginas que no son codigo
			executable->ReadAt(&(machine->mainMemory[pageTable[j].physicalPage*PageSize]), PageSize, noffH.code.inFileAddr+cantLeida);
			cantLeida += PageSize;
		}	
	}
	...
	//anular paginas
    /*for(int k= cantNumPages; k<numPages; ++k){
    	this->AnularPagina(k);
    }
*/
...
// 
//
void AddrSpace::AnularPagina(int index){
	for(int i = 0; i<PageSize; ++i){
		machine->mainMemory[pageTable[index].physicalPage*i]=0;
	}
}
